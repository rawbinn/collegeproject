...
class HomeController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $resultColleges = null;
            ...
            $filter = $request->get('filter') ?? 'Overall';
            if ($filter == 'Placement') {
                $resultColleges = $resultColleges->orderBy('placements', 'DESC');
            } elseif ($filter == 'Budget') {
                $resultColleges = $resultColleges->orderBy('fee_amount', 'ASC');
            }  elseif ($filter == 'Pass') {
                $resultColleges = $resultColleges->orderBy('pass_percent', 'DESC');
            } elseif ($filter == 'Rating') {
                $resultColleges =Rating::join('colleges', 'ratings.college_id', '=', 'colleges.id')
                            ->select(DB::raw('avg(rating_given) as average, colleges.*'))
                            ->groupBy('college_id')
                            ->orderBy('average', 'desc');
            } else {
                $resultColleges = $resultColleges->orderByRaw('((`colleges`.`pass_percent`*0.3)+(`colleges`.`extra_activities`*0.2)+(`colleges`.`placements`*0.5)) DESC');
            }

            $resultColleges = $resultColleges->paginate(10);

            return view('search', compact('levels', 'affiliations', 'faculties', 'colleges', 'locations', 'request', 'resultColleges'));
    }
